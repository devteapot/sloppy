...
export function assertAcpSpawnAllowed(options: {
  ...
  if (!capabilities) {
    throw new LlmConfigurationError(
      `ACP adapter '${options.adapterId}' has no capabilities declaration. Routed or allow-masked ACP spawns require plugins.delegation.acp.adapters.${options.adapterId}.capabilities.`,
    );
  }
