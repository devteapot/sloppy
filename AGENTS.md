...
- `docs/18-voice-plugin.md` for the consolidated voice Plugin, immutable run plans, audio adapters, and privacy seam.
...
- Native LLM profiles share durable provider-neutral conversation history with safe semantic compaction, normalized one-shot context-overflow recovery, fresh SLOP state tails, and continuity across profile replacement and Session restore. ACP profiles remain external session boundaries and own their internal conversation policy.
- Default Plugins now also include `images`; the opt-in consolidated `voice` Plugin owns its speech-profile Provider and optional `/conversation` Session facet. Voice freezes STT/TTS destinations before audio acquisition, requires explicit non-auto-approvable approval before remote egress, publishes live captions and resource ownership through non-durable typed `pluginState`, and atomically queues finalized transcripts when a Session is busy.
...
- The typed session snapshot exposes generic extension metadata, non-durable Plugin-scoped `pluginState`, and client-agnostic plugin contributions. Its compact SLOP projection exposes `/extensions`, plugin ownership metadata at `/plugins`, and deliberate feature projections such as opt-in `/goal` and `/conversation`. Plugin-owned client actions invoke typed plugin commands; presentation hints may include TUI slash metadata but execution must not branch on the TUI. Keep reusable goal strategy in the `persistent-goal` skill; do not add planner-specific DAG policy to core.
