...
- The current implementation includes built-in `terminal`, `filesystem`, `orchestration`, `spec`, and `delegation` providers, a consumer hub, dynamic affordance tools, fixed observation tools, a native Anthropic adapter, a native Gemini adapter, an OpenAI-compatible adapter for OpenAI, OpenRouter, and Ollama, an optional ACP-backed `SessionAgent` path for delegated third-party child agents, and a managed LLM-profile layer with secure credential storage for macOS and Linux. The orchestration provider supports durable plan-scoped task DAGs, cycle rejection, spec refs, audit findings, typed handoffs, scoped child work packets, parallel-friendly dependency inference for common coding plans, scheduler-claimed ready tasks, pushed child results, verification-gated completion, and additive docs/12 HITL artifacts: goals with minor/material revision gates, gates, messages, plan revisions, typed evidence claims, evidence blobs, stale-spec guards, scoped gate-policy defaults, deterministic policy-resolved slice gates plus narrow goal/plan auto-resolution, opted-in precedent-resolved `SpecQuestion`s with semantic precedent matching and injected or LLM-backed borderline tie-breaks, active persisted progress/coherence/intent drift events, intent-drift coverage gates, blast-radius and irreversible-action guardrails, allowlisted final-audit replay, configured wall-time/retry/token/cost budget gates with raise-cap affordances, typed digest generation with action refs and opt-in generic/Slack/email push-delivery outbox dispatch, and durable precedent/case-record matching. The spec provider supports goal/version refs, accepted specs, immutable spec version snapshots, and requirement criterion metadata.
- The current checked-in interfaces are a CLI/REPL, a headless `src/session/` agent-session surface with `/llm`, `/apps`, and `/orchestration` controls including `start_spec_driven_goal`, latest digest actions, gate controls, and drift/progress summaries, a Go TUI under `apps/tui/` that can onboard/manage LLM profiles, inspect external app attachment state, and act on orchestration gates/digest actions, and a canvas/HTML dashboard under `apps/dashboard/` that visualizes orchestration files, typed digests, and optional session-provider-backed digest actions.
...
bun test tests/docs12-orchestration.test.ts
```
